...
def main():
    ...
    if len(sys.argv) < 2:
        sys.exit("Usage: python questions.py corpus stopwords")

    # Calculate IDF values across files
    files = load_files(sys.argv[1])

    # stop words selector 
    if sys.argv[2] is not None and sys.argv[2] == 'ukr':
        stop = load_stopwords_text(sys.argv[2])
        # print('Ukranian stopwords is loaded')
    else:
        stop = stopwords.words("english")
        print('English stopwords')

    file_words = {
        filename: tokenize(files[filename], stop)
        for filename in files
    }
    file_idfs = compute_idfs(file_words)

    # Prompt user for query
    query = set(tokenize(input("Query: "), stop))
    ...
    print('------- ')
    print(f'Question was {str(query)}')

    main()

def load_stopwords_text(directory):
    ...
    for file in os.listdir(directory):
        ...
        if file.endswith('.txt'):
            try:
                ...
                print(f'problem with stopwords file {file}')
                print(str(e))
                print('not loaded urk stopwords')

    # print('loaded urk stopwords')
    print()
    return res
...
def tokenize(document, stop_words):
    """
    ...
    return [word.lower() for word in tokens if not all(f in string.punctuation for f in word) and word.lower() not in stop_words]
...
def top_sentences(query, sentences, idfs, n):
    ...
    """
    # print('looking for top sentences')
